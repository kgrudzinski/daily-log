...
export function Menu({ selected, onChange, children }) {
  ...
  return (
    <MenuContext.Provider
      value={{ selectedItem: selectedItem, onSelectItem: onSelectItem }}
    >
...
export function MenuHeader({ children }) {
  return <div className="app-menu-header">{children}</div>;
}
...
export function MenuItems({ children }) {
  return <ul className="app-menu-list">{children}</ul>;
}
...
export function MenuItem({ value, label }) {
  ...
  return (
    <li className={item_class} onClick={() => onSelectItem(value)}>
      {label}
    </li>
  );
}
